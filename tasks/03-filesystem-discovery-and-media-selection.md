...
Implement safe, deterministic discovery of source folders and `.mkv` files, together with the interactive selections that determine what the later planning task will process. The source root is the current working directory from which the executable was started. The destination is chosen after startup TMDB configuration and before source folders are listed.
...
5. for each selected folder, list direct `.mkv` files;
...
The UI may show metadata such as file size and relative path, but the selection result must retain the exact source path needed for later revalidation.
...
For every selected source folder, list only direct regular files whose extension is `.mkv`, case-insensitively. `.mkv`, `.MKV`, `.Mkv`, and equivalent case variants are eligible.

Rules:

- do not recurse into nested folders;
- do not follow symbolic links;
- do not include directories with an `.mkv` suffix;
- sort deterministically;
- show at least the filename, with size or relative path when useful;
...
SelectedSource {
    folder: path,
    files: list of selected regular .mkv paths,
}
```
...
- support for extensions other than `.mkv`.
...
- case-insensitive `.mkv` matching;
- nested `.mkv` exclusion;
...
- [ ] Filesystem discovery occurs only after TMDB key and language validation.
- [ ] The current working directory is the source root.
- [ ] The destination is requested before source folders are listed.
- [ ] Existing directories are accepted and existing files are rejected as destinations.
- [ ] The current directory and selected source folders cannot be used as the destination.
- [ ] A nonexistent destination is not created during input.
- [ ] Only direct child source folders are listed.
- [ ] The destination is excluded from source choices when it is inside the current directory.
- [ ] Symbolic links and nested folders are excluded in the MVP.
- [ ] Only direct regular `.mkv` files, case-insensitively, are listed.
- [ ] Multiple source folders and multiple files per folder can be selected explicitly.
- [ ] The first item is never selected automatically.
- [ ] Empty and invalid states have clear English recovery/cancellation paths.
- [ ] Discovery returns typed paths associated with their source folders.
- [ ] Automated tests cover path safety, filtering, sorting, and cancellation.
